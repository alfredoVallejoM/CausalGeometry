...
/-- Alexander--Whitney style causal cubical cup value.
...
/-- Bilinear graded cup product on raw causal cubical cochains. -/
def cup
    (p q : ℕ) :
  ...
  toFun := fun α =>
    { toFun := fun β Q =>
      ...
      map_add' := by
        intro β γ
        funext Q
        simp [cupValue, mul_add]
      map_smul' := by
        intro a β
        funext Q
        simp [cupValue, mul_assoc] }
  map_add' := by
    intro α γ
    apply LinearMap.ext
    intro β
    funext Q
    simp [cupValue, add_mul]
  map_smul' := by
    intro a α
    apply LinearMap.ext
    intro β
    funext Q
    simp [cupValue, mul_assoc, mul_comm,
      mul_left_comm]

@[simp] theorem cup_apply
    (p q : ℕ)
    (α : CausalCubicalCochain S K p)
    (β : CausalCubicalCochain S K q)
    (Q : CausalEventCube S (p + q)) :
...
@[simp] theorem cup_zero_zero_apply
...
multiplication. -/
theorem cup_configuration_observables
...
theorem differential_zero_cup_configuration
    ...
    causalDifference
      (fun X => F X * G X)
      C e h := by
  rw [cup_configuration_observables]
  exact
    differential_zero_recovers_causalDifference
      (fun X => F X * G X)
      C e h

/-- Chronological finite product rule recovered from the cubical cup.

This is a low-degree regression for the future all-degree graded Leibniz
theorem. -/
theorem differential_zero_cup_product_rule
    (F G : Configuration S → K)
    (C : Configuration S)
    (e : Event)
    (h : S.Enabled C e) :
    ...
    causalDifference F C e h *
        G C := by
  rw [differential_zero_cup_configuration]
  exact causalDifference_mul F G C e h

/-- Alexander--Whitney degree-zero Leibniz form.
...
Leibniz rule. -/
theorem differential_zero_cup_leibniz
    (F G : Configuration S → K)
    (C : Configuration S)
    (e : Event)
    (h : S.Enabled C e) :
